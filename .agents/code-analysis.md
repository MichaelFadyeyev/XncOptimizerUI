...
Bores from every applied `XncProgram` (`AppViewModel.SelectedXncPrograms`) are drawn on top of the geometry by `BorePreviewRenderer.DrawBores`: a face bore (`XncBore.Surface == Face`) gets a circle + crossed center lines on the Face rectangle plus a rectangle + center line on each of the four side bands, colored by the owning program's `Side`; an edge bore (`Top`/`Bottom`/`Left`/`Right`) gets a circle + crossed center lines on the one band it was drilled from (positioned from `bore.Z`) plus a rectangle + center line on the Face rectangle. All five projections of one bore toggle red together on click (selection is local to the current render pass, not yet wired into `AppViewModel`). Groovings are drawn the same way by `GroovePreviewRenderer.DrawGrooves`, sharing band/Face rectangle placement math with `BorePreviewRenderer` via `PartPreviewOverlayGeometry`. Unlike a bore, a groove's perpendicular offset always measures from the target band's inner (Face-adjacent) edge regardless of the owning program's `Side` (confirmed with the user - every groove-side `AddSideRectangleRange`/`Z`-anchor call passes a literal `true`, not `program.Side`). A front-plane groove (`SideCode == 0`) gets a Face rectangle (length between its start/end points, width `t`, offset from that line per its `c` tool-position), plus orientation-aware side-band projections - the two bands parallel to the groove get a plain `dp`-deep extent projection with no center line, the two perpendicular bands get a small `t`-wide/`dp`-deep rectangle at the groove's constant coordinate with a center line (a diagonal groove falls back to extent-with-center-line on all four). An edge-plane groove (`SideCode` `1`-`4` = Left/Right/Top/Bottom) gets its true rectangle on the one band it belongs to, offset from the band's inner edge by its own `Z` attribute (not by its `Start`/`End` coordinate, which is just the panel-edge value); a `dp` rectangle (no center line) on the Face rectangle; and a small `t`-wide/`dp`-long marker (with a center line) on each of the two bands perpendicular to its own plane - flush at its own fixed edge coordinate along its intrinsic depth axis, and displaced from *that target band's* inner edge by the groove's own `Z` (same formula/helper as its own-band rectangle). Grooves are also still shown in the "Grooves" `DataGrid` (below), whose Side column uses the same `SideCode` 1=Left/2=Right/3=Top/4=Bottom mapping. Milling contours and pockets are **not** rendered on the canvas, nor is banding; see `.agents/todos.md` for what's still deferred.
