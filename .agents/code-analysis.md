...
- **`MainWindow.xaml`** — 3-column layout: command buttons + label management (left), Bands `DataGrid` (top-right), filterable Parts `DataGrid` (row 1), a half-height part-preview row (row 2: a `PartCanvas` viewport that fills the width + a `SelectedPartPrograms` `TextBox` 350 px wide on its right), and the log box (row 3). Icon `xnc_logo.ico`. Part-preview colours / line widths / metrics are `Window.Resources` keys (`PartFaceBrush`, `PartOutlineBrush`, `PartOutlineThickness`, `MinEdgeProjectionThickness`, `ProjectionGapMm` — the 10 mm face-to-side-view gap, scaled with the part, `PartPreviewMargin`, `AxisXBrush`, `AxisYBrush`, ...). Canvas drawing lives in `MainWindow.xaml.cs` (`RenderPart`), re-run on `SelectedPart` change and canvas resize; see `.agents/todos.md` for deferred hooks (click hit-testing via shape `Tag`, machining overlay).

# User-facing operations

**File lifecycle** — Open/Save/Close project XML file. Opening populates Bands, Sheets, Parts grids from the parsed file and pre-selects the first part. Closing clears all state. Window title shows assembly name + current filename.

**Part preview** — the half-height row under the Parts grid draws the selected part in a `Canvas` viewport: a plain rectangle at `Length x Width` plus the four side views of the part. Each side view is a band whose thickness is the owning sheet's `t` (looked up from `AppViewModel.Sheets` by `PartVM.SheetId`) and whose length is the part's size in that direction, offset from the face by a 10 mm gap; band thickness, gap and face are all drawn at one uniform scale that fits face + gap + band per side into the viewport (rescaled per part). A small X/Y axis glyph sits top-left. Redrawn whenever `SelectedPart` changes or the canvas resizes. First iteration is geometry only — no bores / mills / groovings / pockets / banding are rendered (see `.agents/todos.md`).
